			...
			<div class="col-12">
				...
				<div class="card p-3 mt-3">
					<div
						...
						<h5 class="mb-0">Your Users</h5>

						<!-- Search Form -->
						<form class="d-flex" method="get"
							action="ManageUsersController">
							<input type="hidden" name="action" value="fetchAllUsers" /> <input
					...
					<div class="card-body p-0">
						<table class="table table-striped mb-0">
							...
							<tbody>
								<c:choose>
									<c:when test="${not empty parentUserList}">
										<c:forEach var="p" items="${parentUserList}"
											varStatus="loop">
											<tr>
												<td>${loop.index + 1}</td>
												<td>${p.parentName}</td>
												<td>${p.email}</td>
												<td><a
													href="ManageUsersController?action=viewUserDetails&userId=${p.parentId}"
